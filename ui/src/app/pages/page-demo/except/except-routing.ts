...
export default [
  ...
  { path: 'network-error', title: 'Lỗi mạng', data: { key: 'network-error' }, loadComponent: () => import('./network-error/network-error.component').then(m => m.NetworkErrorComponent) },
  { path: 'no-data', title: 'Không có dữ liệu', data: { key: 'no-data' }, loadComponent: () => import('./no-data/no-data.component').then(m => m.NoDataComponent) }
] satisfies Route[];
